...
private enum RecoveryMonitor {
    ...
    static func markCleanBackground() {
...
struct CinaVaultIOSApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            CinaVaultRootView(model: model, recoveryDiagnostic: $recoveryDiagnostic)
                .preferredColorScheme(.dark)
                .background(CVColor.ink)
                .onChange(of: scenePhase) { _, phase in
                    switch phase {
                    case .active:
                        RecoveryMonitor.markActive()
                    case .background:
                        RecoveryMonitor.markCleanBackground()
                    case .inactive:
                        break
                    @unknown default:
                        break
